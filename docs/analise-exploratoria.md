...
Texto de **Ana Caroline Amorim**, a partir dos notebooks `01_exploracao_inicial.ipynb` e `02_eda.ipynb`. No fim da página há um complemento (Spearman, Cramér, Tukey e PCA) que entrou depois, quando a preparação e a modelagem já estavam andando.

| Item | Descrição |
|---|---|
| **Disciplina** | Aprendizagem de Máquina Não Supervisionado — MBA (UNIFOR) |
| **Tema** | Tema 08 — RH e People Analytics |
| **Etapa** | Análise exploratória (antes da clusterização) |
| **Base de dados** | `WA_Fn-UseC_-HR-Employee-Attrition.csv` — 1.470 colaboradores × 35 variáveis |
| **Notebooks** | `01_exploracao_inicial.ipynb` e `02_eda.ipynb` |

---

## 1. Introdução e Objetivo da Fase

Este relatório reúne a análise exploratória da base IBM HR Analytics Employee Attrition, etapa anterior à clusterização (ver [Canvas do Problema](canvas_problema.md)). O objetivo é mapear estrutura, qualidade e comportamento das variáveis, e reunir evidências sobre rotatividade (`Attrition`) e desempenho (`PerformanceRating`) para a modelagem.

`Attrition` e `PerformanceRating` entram só como leitura de negócio. **Não** vão para o treino dos grupos; servem depois para ver se os clusters separam saída e nota.
...
![Distribuição das escalas Likert](assets/figures/02_likert.png)
...
![Attrition por OverTime e Departamento](assets/figures/07_attrition_overtime_dept.png)

*Figura 4 — Taxa de Attrition por OverTime e por Departamento*

### 7.2 Variáveis numéricas: quem sai é diferente de quem fica?

![Boxplots por Attrition](assets/figures/04_boxplot_attrition.png)
...
![Mapa de correlação](assets/figures/06_correlacao.png)
...
### Spearman, Cramér e discrepantes

Pearson no heatmap já mostra o bloco renda/cargo. Spearman no mesmo par `MonthlyIncome` × `JobLevel` dá ρ = 0,92 — não é só relação linear. Entre nominais, `Department` × `JobRole` tem V de Cramér ≈ 0,94: cargo e departamento carregam quase a mesma informação (Sales Executive está em Sales; faz sentido).

Na regra de Tukey (1,5×IQR), 114 pessoas (7,8%) ficam na cauda de `MonthlyIncome` e 104 (7,1%) em `YearsAtCompany`. Não tratamos isso como erro de cadastro: é o pessoal sênior puxando a média. Decisão na preparação: **manter**. Tabelas: `reports/tables/eda_spearman_pares.csv`, `eda_cramer_pares.csv`, `eda_discrepantes_tukey.csv`.

### PCA só para ver o desenho

Dois componentes principais explicam cerca de 29% da variância. O problema não cabe num plano 2D. Por isso o PCA ficou como diagnóstico; o espaço que usamos na modelagem é Gower.

![PCA exploratório](assets/figures/15_pca_exploratorio.png)

*Figura 8 — Projeção PCA exploratória (não é o espaço de produção)*
...
### O que veio depois deste relatório

A preparação removeu as constantes e o ID, reservou `Attrition` e `PerformanceRating`, manteve os discrepantes e gerou Gower. A clusterização oficial ficou em **k = 2** (K-Medoids). Os dois grupos separam attrition (~11% vs ~21%); hora extra e baixo engajamento não viraram clusters próprios. Ver [preparação](preparacao.md) e [modelagem](modelagem.md).
